...
from openpyxl.styles import Font, Color, Alignment, PatternFill, Alignment, Border, Side, NamedStyle, Alignment
import os

# Load the workbook
wb = load_workbook('/Users/princewill/Downloads/Invoices and Recipts DCBTL/FEB 2024.xlsx', data_only=True)
...
def format_cell(cell, font, fill=None, alignment=None):
        """Format a cell with the given font, fill, and alignment."""
        cell.font = font
        if fill is not None:
            cell.fill = fill
...
for sheet in new_wb.worksheets:
    ...
    # Get the header cell for the 'S/N' column
    header_cell = sheet['A1']

    # Set the value and format the header cell
    header_cell.value = 'S/N'
    format_cell(header_cell, bold_font, amber_fill)

    # Fill the 'S/N' column with sequential numbers and center align the text
    for index, row in enumerate(sheet.iter_rows(min_row=2), start=1):
        serial_number_cell = row[0]
        serial_number_cell.value = index
        format_cell(serial_number_cell, None, None, Alignment(horizontal='center'))

    # Make the title row bold
    for title_cell in sheet[1]:
        format_cell(title_cell, bold_font)
